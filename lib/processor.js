...
class ImageObjectProcessor {
  ...
  process (rawManifest, baseURL, memberName) {
    ...
    function processPurposeMember (image) {
      let value = (image.purpose || '').trim().replace(/\s+/, ' ');
      return value === 'badge' ? value : 'any';
    }
    function processPlatformMember (image) {
      // NOTE: Although the values are not spec'd
      // (https://w3c.github.io/manifest/#n-note10), the Working Group
      // maintains a list of known `platform` values in the spec's Wiki:
      //
      // - https://github.com/w3c/manifest/wiki/Platforms
      let value = (image.platform || '').trim().replace(/\s+/, ' ').toLowerCase();
    ...
    function toImageObject (imageSpec) {
      ...
      const purpose = processPurposeMember(imageSpec);
      const platform = processPlatformMember(imageSpec);
      ...
      if (purpose) {
        imageObject.purpose = purpose;
      }
      if (platform) {
        imageObject.platform = platform;
      }
      if (color) {
...
class ManifestProcessor {
  ...
  process (jsonText, manifestURL, docURL) {
    ...
    if (includeKey('processed_platform_icons')) {
      finalManifest.processed_platform_icons = processPlatformIcons();
    }

    if (includeKey('processed_best_platform_icons')) {
      finalManifest.processed_best_platform_icons = processBestPlatformIcons();
    }
    ...
    function processPlatformIcons () {
      if ('processed_platform_icons' in finalManifest) {
        return finalManifest.processed_platform_icons;
      }
      let platformIcons = {
        'android': [],
        'ios': [],
      ...
      (finalManifest.icons || []).forEach(icon => {
        ...
        if (icon.platform) {
          if (platformIcons[icon.platform]) {
            platformIcons[icon.platform].push(icon);
          } else {
            platformIcons[icon.platform] = [icon];
          }
        }
      });
    ...
    function processBestPlatformIcons () {
      if ('processed_best_platform_icons' in finalManifest) {
        return finalManifest.processed_best_platform_icons;
      }
      let bestPlatformIcons = {};
      let bestBadge = processBestBadge();

      let allPlatformIcons = processPlatformIcons();
      let platformIcons = [];
      Object.keys(allPlatformIcons).forEach(platformKey => {
        platformIcons = allPlatformIcons[platformKey] || [];
        platformIcons.push(bestBadge);

        let bestPlatformIcon = platformIcons[0];
        let iconArea = -1;
        let bestPlatformIconArea = imgObjProcessor.getArea(bestPlatformIcon);
        platformIcons.forEach(icon => {
          iconArea = imgObjProcessor.getArea(icon);
          if (!bestPlatformIcon || iconArea > bestPlatformIconArea) {
            bestPlatformIcon = icon;
            bestPlatformIconArea = iconArea;
          }
        });

        bestPlatformIcons[platformKey] = bestPlatformIcon || null;
      });

      return bestPlatformIcons || allPlatformIcons || {};
    }

    function processBadges () {
      if ('processed_badges' in finalManifest) {
        return finalManifest.processed_badges;
      }
      let badges = [];
      if (finalManifest.icons && finalManifest.icons.length) {
        ...
        for (let i = 0; i < finalManifest.icons.length; i++) {
          ...
          if (icon.purpose && icon.purpose === 'badge') {
            badges.push(icon);
          }
        }
      }
    ...
    function processBestBadge () {
      ...
      const bestIcon = processBestIcon();
      let bestBadge;
      if (finalManifest.icons && finalManifest.icons.length) {
        let iconArea = -1;
        let bestBadgeArea = -1;
        finalManifest.icons.forEach(icon => {
          if (icon.purpose && icon.purpose === 'badge') {
            iconArea = imgObjProcessor.getArea(icon);
            if (!bestBadge || iconArea > bestBadgeArea) {
              bestBadge = icon;
              bestBadgeArea = iconArea;
            }
          }
        });
      }
      return bestBadge || bestIcon || null;
    }
