...
class ManifestProcessor {
  ...
  process (jsonText, manifestURL, docURL) {
    ...
    if (includeKey('processed_document_url')) {
      finalManifest.processed_document_url = processDocumentURL(processedManifest.scope);
    }

    if (includeKey('processed_final_document_url')) {
      finalManifest.processed_final_document_url = processFinalDocumentURL(processedManifest.scope);
    }
    ...
    function processDocumentURL (scope) {
      return urllib.resolve(scope, docURL || finalManifest.start_url);
    }

    function processFinalDocumentURL (scope) {
      return urllib.resolve(scope, docURL);
    }
