...
function autoParse (docURL, manifestFallback) {
...
function autoParseManifest (autoParsedBody) {
  if (autoParsedBody.html) {
    ...
    if (manifests.length) {
      ...
      console.log('!!!', autoParsedBody.manifestFallback);

      let manifestURL = resolveURL(autoParsedBody.docURL, manifests.eq(manifests.length - 1).attr('href'));
      return fetchManifest(manifestURL, autoParsedBody.docURL, autoParsedBody.manifestFallback);
    }
  } else if (autoParsedBody.json) {
    const processor = new ManifestProcessor();
    return processor.process(autoParsedBody.text, autoParsedBody.manifestURL, autoParsedBody.docURL, autoParsedBody.manifestFallback);
  }
...
const fetchManifest = module.exports.fetchManifest = function (manifestURL, docURL, manifestFallback) {
  console.log('~ manifestFallback', manifestFallback);
  if (typeof manifestURL === 'string' &&
      (manifestURL.startsWith('http:') || manifestURL.startsWith('https:'))) {
    return rp({
      uri: manifestURL,
      transform: autoParse(docURL, manifestFallback),
      resolveWithFullResponse: true
    }).then(autoParseManifest);
  } else {
    let manifestJSONOrHTML = arguments[0];
    return Promise.resolve(autoParseManifest(autoParse(docURL, manifestFallback)(manifestJSONOrHTML)));
  }
};
...
module.exports.createServer = function (opts) {
  ...
  function fetchHandler (request, reply, source, error) {
    ...
    next();
    function next () {
      console.log('fetchHandler', error);
      if (error && source === 'query') {
        fixManifestURL(request, reply, true);
        return;
      }
      console.log('??', request.query.manifestFallback);
      fetchManifest(manifestURL, null, request.query.manifestFallback)
        .then(reply)
        .catch(err => {
  ...
  const fetchConfig = {
    validate: {
      query: {
        ...
        manifestFallback: Joi.string()
                .example('{"name: "Example App", "description": "Example description."}')
      },
