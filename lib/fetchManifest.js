  ...
  function fetchHandler (request, reply, source, error) {
    ...
    function next () {
      ...
      fetchManifest(manifestURL)
        .then(manifest => reply(JSON.stringify(manifest, null, request.query.ws)))
        .catch(err => {
          if (err.name === 'StatusCodeError' && err.statusCode === 404 &&
            ...
            return fetchManifest(manifestURL.replace(/^https:/, 'http:'))
              .then(err => reply(JSON.stringify(err, null, request.query.ws)))
              .catch(handleErrors);
          }
      ...
      function handleErrors (err) {
        ...
        reply(JSON.stringify(Boom.badRequest(errMsg), null, request.query.ws));
        return errMsg;
      }
    }
  }
